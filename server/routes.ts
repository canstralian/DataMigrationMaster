...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/migrations/recent", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Failed to fetch recent migration jobs" });
    }
  });
    ...
    try {
      ...
      if (migration.status === "completed" || migration.status === "failed") {
        return res
          .status(400)
          .json({
            message: `Cannot cancel migration in ${migration.status} state`,
          });
      }
    ...
    try {
      ...
      if (!result.success) {
        return res
          .status(400)
          .json({
            message: "Invalid Kaggle credentials",
            errors: result.error.format(),
          });
      }
      ...
      const isValid = await kaggleTransform.validateKaggleCredentials(
        username,
        key,
      );
  ...
  app.post("/api/datasets/:id/transform", async (req, res) => {
    try {
      ...
      const code = kaggleTransform.generateTransformationCode(
        transformations,
        datasetRef,
        fileName,
      );
