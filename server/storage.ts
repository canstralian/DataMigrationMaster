import {
  ...
  migrationSteps,
  type MigrationStep,
  type InsertMigrationStep,
  datasetFiles,
  type DatasetFile,
  type InsertDatasetFile,
  analysisReports,
  type AnalysisReport,
...
export interface IStorage {
  ...
  updateMigrationStepProgress(
    id: number,
    progress: number,
    message?: string,
  ): Promise<MigrationStep | undefined>;
  updateMigrationStepStatus(
    id: number,
    status: string,
    message?: string,
  ): Promise<MigrationStep | undefined>;
...
export class MemStorage implements IStorage {
  ...
  async getMigrationJobsByDatasetId(
    datasetId: number,
  ...
  async createMigrationStep(
    insertStep: InsertMigrationStep,
  ...
  async getDatasetFiles(datasetId: number): Promise<DatasetFile[]> {
    return Array.from(this.datasetFiles.values()).filter(
      (file) => file.datasetId === datasetId,
    );
  }
  ...
  async createAnalysisReport(
    insertReport: InsertAnalysisReport,
  ...
  private seedSampleData() {
    ...
    const dataset1 = this.createDataset({
      ...
      currentUrl:
        "https://huggingface.co/datasets/username/news-classification",
      filesCount: 3,
    ...
    this.updateMigrationStepProgress(
      downloadStep.id,
      67,
      "2 of 3 files downloaded",
    );
    ...
    this.createAnalysisReport({
      ...
      report: {
        summary:
          "High-quality news classification dataset with good coverage across categories",
        issues: [],
        recommendations: [
