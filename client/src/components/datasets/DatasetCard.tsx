...
export default function DatasetCard({ 
  ...
  const hasActiveMigration = migrations?.some(m => m.status === 'in_progress');
  ...
  const getPlatformBadge = (platform: string) => {
    switch (platform) {
      case 'github':
        return <Badge variant="outline" className="bg-neutral-100">GitHub</Badge>;
      case 'kaggle':
        return <Badge variant="outline" className="bg-blue-100">Kaggle</Badge>;
      case 'huggingface':
        return <Badge variant="outline" className="bg-primary/20">Hugging Face</Badge>;
      default:
          ...
          <>
            <div className="border-t border-neutral-200 my-2 pt-2">
              ...
              {migrations && migrations.length > 0 ? (
                    ...
                    <div key={migration.id} className="flex justify-between">
                      <div className="flex items-center">
                        <span className={`h-2 w-2 rounded-full mr-2 ${
                          migration.status === 'completed' ? 'bg-accent' : 
                          migration.status === 'in_progress' ? 'bg-secondary' : 
                          'bg-red-500'
                        }`}></span>
                        {migration.sourcePlatform} → {migration.destinationPlatform}
                      </div>
                ...
                <div className="text-neutral-500 text-sm">No migration history found</div>
              )}
            </div>
