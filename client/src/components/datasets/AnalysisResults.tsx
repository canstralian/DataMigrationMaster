  ...
  const handleGenerateCard = async () => {
    ...
    try {
      const response = await apiRequest(
        "POST",
        `/api/datasets/${datasetId}/generate-card`,
        {},
      );
      ...
      toast({
        ...
        description:
          "The card has been successfully generated based on the analysis.",
      });
    } catch (error) {
      toast({
      ...
      <CardContent>
        ...
        >
              ...
              <Card>
                <CardContent className="pt-6">
                  <div className="text-center">
                    ...
                    <div
                      className={`text-3xl font-bold mb-2 ${getQualityColor(report.quality)}`}
                    >
              ...
              <Card>
                <CardContent className="pt-6">
                  <div className="text-center">
                    ...
                    <div
                      className={`text-3xl font-bold mb-2 ${getQualityColor(report.completeness)}`}
                    >
                      {report.completeness ?? 0}/100
                    </div>
                    <Progress
                      value={report.completeness ?? 0}
                      className="h-2"
                    />
                  </div>
                </CardContent>
              ...
              <Card>
                <CardContent className="pt-6">
                  <div className="text-center">
                    ...
                    <div
                      className={`text-3xl font-bold mb-2 ${getQualityColor(report.usability)}`}
                    >
          ...
          <TabsContent value="metrics" className="space-y-4">
            {reportData.metrics &&
            Object.keys(reportData.metrics).length > 0 ? (
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                        {Object.entries(value as Record<string, any>).map(
                          ([subKey, subValue]) => (
                            <div
                              key={subKey}
                              className="bg-neutral-50 p-2 rounded-md"
                            >
