...
export default function RecentActivity() {
  ...
  const getMigrationTitle = (migration: MigrationJob) => {
    ...
    const source = sourceParts.length >= 2 ? sourceParts[sourceParts.length - 2] + '/' + sourceParts[sourceParts.length - 1] : migration.sourceUrl;
      ...
      <CardContent>
        {isLoading ? (
              ...
              {recentMigrations.map((migration) => (
                <div key={migration.id} className="flex items-start space-x-3">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                    migration.sourcePlatform === 'github' ? 'bg-blue-100 text-blue-700' :
                    migration.sourcePlatform === 'kaggle' ? 'bg-cyan-100 text-cyan-700' :
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      <div className="font-medium text-sm">{getMigrationTitle(migration)}</div>
                    ...
                    {migration.progress !== null && migration.progress > 0 && migration.progress < 100 && (
          ...
          <div className="text-center py-8 bg-neutral-50 rounded-md">
            ...
            <p className="text-xs text-neutral-400 mt-1">Start a migration to see it here</p>
          </div>
        )}
      </CardContent>
    </Card>
