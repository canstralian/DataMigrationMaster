...
export default function MigrationProgress({
  ...
  const { data: migrationJob, isLoading: isLoadingJob } =
    useQuery<ExtendedMigrationJob>({
      queryKey: [`/api/migrations/${migrationId}`],
      refetchInterval: autoRefresh ? 3000 : false,
    });

  // Fetch the migration steps
  const { data: migrationSteps, isLoading: isLoadingSteps } = useQuery<
    MigrationStep[]
  >({
  ...
  useEffect(() => {
    if (
      migrationJob &&
      (migrationJob.status === "completed" || migrationJob.status === "failed")
    ) {
      ...
      if (migrationJob.status === "completed") {
        ...
        toast({
          ...
          description:
            migrationJob.error || "An error occurred during migration.",
          variant: "destructive",
        });
      }
    }
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
              <div>
                ...
                <div className="flex items-center space-x-2">
                  <div
                    className={`w-3 h-3 rounded-full ${getStatusColor(migrationJob.status)}`}
                  ></div>
              ...
              <div>
                ...
                <span className="font-medium">
                  {migrationJob.totalFiles || "0"} files
                </span>
              </div>
