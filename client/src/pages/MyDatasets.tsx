...
export default function MyDatasets() {
  ...
  const deleteMutation = useMutation({
    mutationFn: (datasetId: number) => apiRequest('DELETE', `/api/datasets/${datasetId}`, {}),
    onSuccess: () => {
      toast({
  ...
  const filteredDatasets = datasets
    ?.filter(dataset => {
      ...
      const matchesPlatform = platformFilter === "all" || dataset.currentPlatform === platformFilter;
  ...
  const getDatasetMigrations = (datasetId: number) => {
    return migrations?.filter(migration => migration.datasetId === datasetId) || [];
  };

  // Get analysis reports for a specific dataset
  const getDatasetReports = (datasetId: number) => {
    return analysisReports?.filter(report => report.datasetId === datasetId) || [];
  };
    ...
    <div className="container mx-auto px-4 py-6">
      <div className="mb-8">
        ...
        <p className="text-neutral-600 max-w-3xl">
          Manage, analyze, and migrate your datasets across GitHub, Kaggle, and Hugging Face Hub.
        </p>
      </div>
      ...
      {isLoadingDatasets ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="bg-white rounded-lg shadow-sm border border-neutral-200 p-4 h-64 animate-pulse">
        ...
        <div className="text-center py-12 bg-white rounded-lg shadow-sm border border-neutral-200">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 mx-auto text-neutral-300 mb-4" viewBox="0 0 20 20" fill="currentColor">
          ...
          {searchQuery || platformFilter !== "all" ? (
            ...
            <p className="text-neutral-500 mb-4">
              You haven't imported any datasets yet. Get started by importing a dataset.
            </p>
          )}
          <Button onClick={() => navigate('/migration')}>
            Import Dataset
      ...
      <div className="mt-8 bg-white rounded-lg shadow-sm border border-neutral-200 p-6">
        <h2 className="text-xl font-semibold mb-6">Migration & Analysis History</h2>
            ...
            <table className="min-w-full divide-y divide-neutral-200">
              ...
              <tbody className="bg-white divide-y divide-neutral-200">
                {migrations.slice(0, 5).map((migration) => {
                  const relatedDataset = datasets?.find(d => d.id === migration.datasetId);
                  
                  return (
                    <tr key={migration.id} className="hover:bg-neutral-50">
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-neutral-900">
                        {relatedDataset?.name || `Dataset #${migration.datasetId}`}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-neutral-500">
                        Migration: {migration.sourcePlatform} → {migration.destinationPlatform}
                      </td>
